...
int main(int argc, char** argv) {
    cxxopts::Options options(argv[0], " - ASCII art and color gradient generator");
    options.add_options()
        ("a,ascii", "ASCII art text", cxxopts::value<std::string>())
        ("g,gradient", "Color gradient", cxxopts::value<std::string>())
        ("h,help", "Print usage")
    ;
    ...
    for (unsigned i = 0; i < ascii_art.size(); ++i) {
        ...
        for (int j = 0; j < len; ++j) {
            ...
            std::cout << "\033[38;2;" << color[0] << ";" << color[1] << ";" << color[2] << "m" << ascii_art[i][j];
        }
